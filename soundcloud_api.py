...
class SoundCloudTrack(AttrDict):
    def __init__(self, mapping):
        ...
        try:
            self.artist, self.title = re.findall(split, self['title'])[0]
        except IndexError:
            self.artist, self.title = self['user']['username'], self['title']
    ...
    async def download(self, filepath=None):
        if not filepath:
            filepath = f'downloads/{self.user.username} - {self.title}.mp3'
...
async def main():
    ...
    # query = ' '.join(argv[1:])
    query = 'quix tynan'
    for res in await search(query, limit=2):
        pprint(res)
